-- Crear tabla de órdenes de trabajo (CEA Orders) - Versión Final Corregida
-- Esta tabla almacenará las órdenes de trabajo generadas desde tickets
-- FIXES: Corregidas todas las referencias de schema para usar cea.users y cea.tickets
...
CREATE TABLE IF NOT EXISTS cea.orders (
    ...
    -- Asignación de personal (FIXED: usando cea.users)
    asignado_a UUID REFERENCES cea.users(id), -- Técnico asignado
    supervisor_id UUID REFERENCES cea.users(id), -- Supervisor de la orden
    ...
    -- Seguimiento y escalamiento (FIXED: usando cea.users)
    escalado BOOLEAN DEFAULT FALSE,
    escalado_a UUID REFERENCES cea.users(id), -- A quién se escaló
    fecha_escalado TIMESTAMP WITH TIME ZONE,
    motivo_escalado TEXT,
    
    -- Control de calidad (FIXED: usando cea.users)
    ...
    -- Auditoría (FIXED: usando cea.users)
...
-- Función para calcular SLA deadline automáticamente
...
DROP TRIGGER IF EXISTS calculate_sla_deadline_trigger ON cea.orders;
...
-- Función para actualizar SLA cumplido automáticamente
...
BEGIN
    ...
    IF NEW.estado = 'completada' AND (OLD IS NULL OR OLD.estado != 'completada') AND NEW.sla_cumplido IS NULL THEN
...
-- Trigger para SLA status
DROP TRIGGER IF EXISTS update_orders_sla_status ON cea.orders;
...
-- Ejemplo de inserción de datos (FIXED: usando cea.tickets y cea.users)
/*
...
SELECT 
    (SELECT id FROM cea.tickets LIMIT 1),
    'inspeccion',
    ...
    'media',
    4,
    (SELECT id FROM cea.users WHERE email LIKE '%admin%' LIMIT 1)
WHERE EXISTS (SELECT 1 FROM cea.tickets LIMIT 1)
  AND EXISTS (SELECT 1 FROM cea.users LIMIT 1);
*/
